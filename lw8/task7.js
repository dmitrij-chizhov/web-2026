const minLengthOfPassword = 4;

function generatePassword(length) {
    if (typeof(length) === 'number') {
        if (length < minLengthOfPassword) {
            console.log('Длина пароля должна быть не менее', minLengthOfPassword, 'символов для обеспечения безопасности.');
            return '';
        }
        ...
        console.log('Переданый параметр', length, 'не цифра.');
        return '';
    }   
}

function loopGeneratePassword(count, length) {
    if (typeof(count) === 'number') {
        ...
        console.log('Переданый параметр', count, 'не цифра.');
        return [];
    }
}